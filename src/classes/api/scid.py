...
class API_SCID:
  ...
  def __init__(self, url, path, key, max_records, search, operator, table, page, httpAccept):
    self.url = url
    self.path = path
    self.key = key
  ...
  def getUrl(self, page, pages):
    print(f"Extraindo página {page} de {pages}: SCID")
    r = requests.get(url = self.url + self.path, params = self.payload(start_record = page))
    data = r.json()

    # self.save(data = articles)
    with open(f'./raw/SCID/page_{page}.json', 'w+', encoding='utf-8') as f:
  ...
  def extract(self):
    ...
    data = r.json()
    total_records = data['search-results']['opensearch:totalResults']
